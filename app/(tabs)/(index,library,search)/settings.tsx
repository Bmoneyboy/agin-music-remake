...
import { useCache, useColors, useMemoryCache, usePlaybackHistory, useTabsHeight } from '@lib/hooks';
import { IconBroadcast, IconCircleCheck, IconDoor, IconFileMusic, IconHistory, IconLayoutGrid, IconVolume, IconWifi } from '@tabler/icons-react-native';
...
export type SettingId = 'streaming.maxBitRate' | 'streaming.format' | 'storage.clearCache' | 'developer.copyId' | 'ui.toastPosition' | 'ui.autoFocusSearchBar' | 'app.defaultTab' | 'app.defaultLibraryTab' | 'eq.enabled' | 'downloads.wifiOnly' | 'app.persistQueue' | 'downloads.maxBitRate' | 'downloads.format' | 'history.enabled' | 'history.scrobbleToServer' | 'history.clear';
            ...
            <ScrollView contentContainerStyle={{ paddingBottom: tabsHeight }}>
                <View style={styles.settings}>
                    ...
                    <Setting
                        ...
                        options={formatOptions}
                    />
                    <SettingsSection label='Playback History' />
                    <Setting
                        id='history.enabled'
                        type='switch'
                        label='Track Playback History'
                        description='Store what you listen to on this device'
                        icon={IconHistory}
                        defaultValue={true}
                    />
                    <Setting
                        id='history.scrobbleToServer'
                        type='switch'
                        label='Scrobble to Server'
                        description='Submit plays to your server. Queued while offline'
                        icon={IconBroadcast}
                        defaultValue={true}
                    />
                    <Setting
                        ...
                        description='Deletes local history and any pending scrobbles'
                        onPress={async () => {
                            Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Heavy);
                            const confirmed = await SheetManager.show('confirm', {
                                payload: {
                                    title: 'Clear Playback History',
                                    message: 'Are you sure? This only affects history stored on this device.',
                                    confirmText: 'Clear',
                                    cancelText: 'Cancel',
                                }
                            });
                            ...
                            await playbackHistory.clearAll();
                        }}
                    />
